...
| 0c | STT engine options on device? | `SpeechRecognition`/`webkitSpeechRecognition` **present**, `getUserMedia` present, secure context ok. WebAssembly was disabled browser-wide (Bromite hardening); **user enabled WASM + JIT via flags 2026-08-20 — verified: compile ✓, SIMD ✓**. **Engine decision (2026-08-20): [transcribe.cpp](https://github.com/handy-computer/transcribe.cpp) via a Node sidecar on the daemon host** (tailnet WebSocket; browser AudioWorklet streams 16 kHz float32 PCM; sidecar returns committed/tentative partials). No browser/WASM build exists (TS binding is Node koffi FFI); Pages-origin speech follows the same tailnet path as the daemon per 0d. Native SpeechRecognition = no-sidecar fallback; whisper.cpp WASM = last resort. | 2026-08-20 |
| 0d | Pages → local daemon connectivity? | **Blocked from Pages** (HTTPS origin throws on `ws://` — mixed content). **Solved for the dev path 2026-08-20**: pad page at `http://localhost:5173` connects to `ws://100.127.193.39:6767/ws` after adding `daemon.cors.allowedOrigins: ["http://localhost:5173"]` to `~/.paseo/config.json` + `paseo reload` (runtime-safe). Verified on device: WS opens, live agents/workspaces render. | 2026-08-20 |
