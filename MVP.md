...
| 0c | STT engine options on device? | `SpeechRecognition`/`webkitSpeechRecognition` **present**, `getUserMedia` present, secure context ok. WebAssembly was disabled browser-wide (Bromite hardening); **user enabled WASM + JIT via flags 2026-08-20 — verified: compile ✓, SIMD ✓**. STT options for Stage 5: native SpeechRecognition (live mic test pending), whisper.cpp WASM (viable again), cloud STT via fetch. | 2026-08-20 |
